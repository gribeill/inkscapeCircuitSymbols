...
SmDeltaChar=u'\u03B4'.encode('utf-8')

# package needed:  steinmetz

def addpt(start=[0,0], rel=[0,0]):
    return [start[0]+rel[0], start[1]+rel[1]]
...
class CircuitSymbols(inkBase.inkscapeMadeEasy):
  def __init__(self):
    ...
    self.OptionParser.add_option("--bipoleRLCVal", action="store", type="string", dest="bipoleRLCVal", default=None)
    ...
    self.OptionParser.add_option("--uwType",action="store", type="string",dest="uwType", default='tline')
    self.OptionParser.add_option("--uwVal", action="store", type="string", dest="uwVal", default=None)
    self.OptionParser.add_option("--uwUnit",action="store", type="inkbool",dest="uwUnit", default=False)
    self.OptionParser.add_option("--uwRot", action="store", type="string", dest="uwRot", default='0')
  ...
  def effect(self):
    ...
    if so.tab=='RLC':
      ...
      if so.bipoleRLC=="jj":
        if so.bipoleRLCUnit:
          if inkDraw.useLatex:
            so.bipoleRLCVal+=r'\si\ampere'
          else:
            so.bipoleRLCVal+= 'A'
        self.drawJJ(root_layer,position,value=so.bipoleRLCVal,angleDeg=so.bipoleRLCRot,
                          flagVolt=so.bipoleRLCVolt,voltName=so.bipoleRLCVoltName,flagCurr=so.bipoleRLCCurr,
                          currName=so.bipoleRLCCurrName,invertArrows=so.bipoleRLCVoltCurrInvert)

      if so.bipoleRLC=="squid":
        self.drawSQUID(root_layer,position,value=so.bipoleRLCVal,angleDeg=so.bipoleRLCRot,
                          flagVolt=so.bipoleRLCVolt,voltName=so.bipoleRLCVoltName,flagCurr=so.bipoleRLCCurr,
                          currName=so.bipoleRLCCurrName,invertArrows=so.bipoleRLCVoltCurrInvert)
    ...
    if so.tab=='Microwave':
      ...
      if so.uwUnit and inkDraw.useLatex:
          so.uwVal=latexUnitMultiple(so.uwVal)
      ...
      if so.uwType=="tline":
        if so.uwUnit:
          if inkDraw.useLatex:
            ...
            so.uwVal+= OhmChar
        self.drawTline(root_layer,position,value=so.uwVal,angleDeg=so.uwRot)
  ...
  def drawTline(self,parent,position=[0, 0],value='Z_0',label='Tline',angleDeg=0, addGnd=False):

    """ draws a transmission line

    parent: parent object
    position: position [x,y]
    value: string with resistor value. (default L'Z_0')

    label: label of the object (it can be repeated)
    angleDeg: rotation angle in degrees counter-clockwise (default 0)
    """
    grey = inkDraw.color.gray(0.75)
    lsBlackFill=inkDraw.lineStyle.set(lineWidth=0.7,fillColor=inkDraw.color.defined('black'))
    lsGreyFill=inkDraw.lineStyle.set(lineWidth=0.7,fillColor=grey,lineColor=grey)
    lsGreyLine= inkDraw.lineStyle.set(lineColor=grey)
    group = self.createGroup(parent,label)
    elem = self.createGroup(group)

    tl_len = 125.0 #length of line
    tl_r = 8.0 #TL radius
    tl_seg = 8.0 #first and last segment of wire

    #first draw the light grey stuff so it looks right
    p = position
    dx = [tl_len, 0.]
    p = addpt(p, dx)
    inkDraw.circle.centerRadius(elem, p, 1.0, lineStyle=lsGreyFill)
    inkDraw.ellipse.centerStartEndAngles(elem, p, tl_r/3.0, tl_r, 90.0, 270.0, largeArc=False, lineStyle=lsGreyLine)
    inkDraw.line.relCoords(elem, [[-tl_len, 0]], p, lineStyle=lsGreyLine)
    inkDraw.line.relCoords(elem, [[tl_r/3.0, 0.]], p, lineStyle=lsGreyLine)
    #go back to start and draw black bits
    p = position
    dx = [tl_seg, 0.]
    inkDraw.line.relCoords(elem, [[-tl_seg, 0.]], p)
    inkDraw.line.relCoords(elem, [[tl_len, 0.]], addpt(p, [0., tl_r]))
    inkDraw.line.relCoords(elem, [[tl_len, 0.]], addpt(p, [0., -1.0*tl_r]))
    inkDraw.ellipse.centerRadius(elem, p, tl_r/3, tl_r)
    inkDraw.circle.centerRadius(elem, p, 1.0, lineStyle=lsBlackFill)
    p = addpt(p, [tl_len, 0.])
    inkDraw.ellipse.centerStartEndAngles(elem, p, tl_r/3.0, tl_r, 270.0, 90.0, largeArc=False)
    inkDraw.line.relCoords(elem, [[tl_seg-tl_r/3, 0.]], addpt(p, [tl_r/3.0, 0]))

    if addGnd:
        p = position
        p = addpt(p, [tl_len*0.75, tl_r])
        inkDraw.line.relCoords(elem, [[0.0, tl_r*0.6]], p)
        p = addpt(p, [-0.3*tl_r, tl_r*0.6])
        inkDraw.line.relCoords(elem, [[0.6*tl_r, 0.]], p)


    pos_text=[position[0]+tl_len/2,position[1]-1.2*tl_r-self.textOffset]
  ...
  def drawResistor(self,parent,position=[0, 0],value='R',label='Resistor',angleDeg=0,
    ...
    #---------------------------------------------
  def drawJJ(self, parent, position=[0, 0], value='B', label='JJ', angleDeg=0,
       flagVolt=True, voltName='v', flagCurr=True,currName='i', invertArrows=False):
    ...
    center = [position[0]+25, position[1]]
    inkDraw.line.relCoords(elem, [[50,0]] , position)
    inkDraw.line.relCoords(elem, [[-6, -6], [12,12]] , center)
    inkDraw.line.relCoords(elem, [[-6, 6], [12,-12]] , center)

    pos_text=[position[0]+25,position[1]-7-self.textOffset]
    ...
    if flagCurr:
      self.drawCurrArrow(group,[position[0]+40 ,position[1]-9],name=currName,color=self.currentColor,angleDeg=angleDeg,invertArrows=invertArrows)

    return group;

  def drawSQUID(self, parent, position=[0, 0], value='BB', label='SQ', angleDeg=0,
       flagVolt=True, voltName='v', flagCurr=True,currName='i', invertArrows=False):
    """ draws a SQUID loop
    ...
    jj1pos = [center[0], center[1]-R]
    jj2pos = [center[0], center[1]+R]

    jjsz = 4.0
    jj1pos1 = [jj1pos[0]-jjsz, jj1pos[1]-jjsz]
    jj1pos2 = [jj1pos[0]-jjsz, jj1pos[1]+jjsz]

    jj2pos1 = [jj2pos[0]-jjsz, jj2pos[1]-jjsz]
    jj2pos2 = [jj2pos[0]-jjsz, jj2pos[1]+jjsz]

    #SQUID loop circle and connecting arms
    inkDraw.circle.centerRadius(elem, center, R)
    inkDraw.line.relCoords(elem, [[25-R,0]] , position)
    inkDraw.line.relCoords(elem, [[25-R,0]] , [center[0]+R, center[1]])
    #arm to loop dots
    inkDraw.circle.centerRadius(elem, [center[0]-R, center[1]], 1.0, lineStyle=linestyleBlackFill)
    inkDraw.circle.centerRadius(elem, [center[0]+R, center[1]], 1.0, lineStyle=linestyleBlackFill)

    #JJs
    inkDraw.line.relCoords(elem, [[2*jjsz, 2*jjsz]] , jj1pos1)
    inkDraw.line.relCoords(elem, [[2*jjsz, -2*jjsz]] , jj1pos2)
    inkDraw.line.relCoords(elem, [[2*jjsz, 2*jjsz]] , jj2pos1)
    inkDraw.line.relCoords(elem, [[2*jjsz, -2*jjsz]] , jj2pos2)

    pos_text=[position[0]+25,position[1]-20-self.textOffset]
  ...
  def drawCapacitor(self,parent,position=[0, 0],value='C',label='Capacitor',flagPol=False,angleDeg=0,
    ...
    inkDraw.line.relCoords(elem, [[10,0]],position)
    inkDraw.line.relCoords(elem, [[-10,0]],[position[0]+24,position[1]])
    inkDraw.line.relCoords(elem, [[0,-14]],[position[0]+10,position[1]+7])
    inkDraw.line.relCoords(elem, [[0,-14]],[position[0]+14,position[1]+7])
